					...
					<div className="w-1/4">
						<Form method="get" className="space-y-4">
							<Accordion type="multiple" className="w-full">
								<RenderFilters filterInputs={loaderData.filterSchema} />
							</Accordion>
						</Form>
					</div>
				)}
...
const RenderFilters = ({
	...
	filterInputs: Route.ComponentProps['loaderData']['filterSchema']
}) => {
	const [searchParams, setSearchParams] = useSearchParams()
	const capacityCheckboxRef = React.useRef<Record<string, boolean>>({})

	const isFilterChecked = React.useCallback(
		(filterCategory: string, filterName: string, params: URLSearchParams) => {
			if (filterCategory === 'Capacity') {
				const min = parseInt(params.get('minCapacity') || '0')
				const maxStr = params.get('maxCapacity')
			...
			return params.getAll(paramName).includes(filterName)
		},
		[],
	)

	const handleCheckboxChange = React.useCallback(
		(filterCategory: string, filterName: string, value: boolean | string) => {
					...
					<AccordionItem key={option.title} value={option.value}>
						...
						<AccordionContent className="flex flex-col gap-4 px-2 text-balance">
							{option.inputs.map((input) => {
								...
								const isChecked =
									input.type === 'checkbox'
										? isFilterChecked(option.title, input.name, searchParams)
										: undefined
								return (
